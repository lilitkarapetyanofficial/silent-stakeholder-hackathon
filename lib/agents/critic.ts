...
export async function runCritic(
  ...
  for (const gap of verifiedGaps) {
    const evidenceReviewTexts = gap.evidence.reviews
      .map((r) => {
    ...
    const evidenceIssueTexts = gap.evidence.issues
      .map((i) => {
...
${evidenceReviewTexts || "No review evidence"}

EVIDENCE ISSUES (${gap.evidence.issues.length} issues):
${evidenceIssueTexts || "No issue evidence"}
